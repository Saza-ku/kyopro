...
/////////////////⊂('ω'⊂ )))Σ≡GO!/////////////////
int N;
void dijkstra(vector<vector<pair<int, int>>> graph, vector<int> &d, int start){
...
int main() {
  ...
  vector<int> d_1(N, 1e9);
  vector<int> d_2(N, 1e9);

  dijkstra(graph, d_1, 0);
  dijkstra(graph, d_2, N-1);

  rep(i, N){
      cout << d_1[i] + d_2[i] << endl;
  }
}